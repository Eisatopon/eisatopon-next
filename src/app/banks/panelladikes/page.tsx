...
const SchoolBadge = ({ school, exam }: { school: string; exam: string }) => {
  ...
  if (school === "omogeneis" || exam === "omogeneis") { text = "ΟΜΟΓΕΝΕΙΣ"; color = "bg-cat-amber/10 text-cat-amber border-cat-amber/30"; }
  else if (exam === "gel_repeat") { text = "ΓΕΛ ΕΠΑΝΑΛΗΠΤΙΚΕΣ"; color = "bg-cat-green/10 text-cat-green border-cat-green/30"; }
  else if (exam === "epal_repeat") { text = "ΕΠΑΛ ΕΠΑΝΑΛΗΠΤΙΚΕΣ"; color = "bg-cat-red/10 text-cat-red border-cat-red/30"; }
  else if (school === "epal") { color = "bg-cat-purple/10 text-cat-purple border-cat-purple/30"; }
  return <span className={`px-2.5 py-1 rounded-full text-[0.65rem] font-bold border ${color}`}>{text}</span>;
};
...
export default function PanelladikesPage() {
  ...
  const loadYear = useCallback(async (year: string): Promise<PanelladikoProblem[] | false> => {
    if (loadedYearRef.current === year && allProblemsRef.current.length > 0) return allProblemsRef.current;
    const url = DATA_URLS[year];
    if (!url) { setInfo(`❌ Δεν υπάρχουν δεδομένα για το έτος ${year}`); return false; }
    setLoading(true);
  ...
  const applyFilters = useCallback((problems: PanelladikoProblem[], school: string, subject: string, topic: string, search: string) => {
    const filtered = problems.filter((q) => {
      ...
      if (search) {
        ...
        if (!q.content.toLowerCase().includes(query) && !q.topic.toLowerCase().includes(query)) return false;
      }
      return true;
    });
  ...
  const handleSearch = async () => {
    if (!selectedYear) { setInfo("⚠️ Επιλέξτε έτος"); return; }
    const problems = await loadYear(selectedYear);
    if (problems) applyFilters(problems, selectedSchool, selectedSubject, selectedTopic, searchQuery);
  };

  const handleReset = () => {
    setSelectedYear(""); setSelectedSchool(""); setSelectedSubject(""); setSelectedTopic(""); setSearchQuery("");
  ...
  const generateQR = () => {
    ...
    setQrUrl(`https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${encodeURIComponent(`${base}?questions=${ids}&year=${loadedYear}`)}`);
    setShowQRModal(true);
  };
  ...
  useEffect(() => {
    ...
    const ids = params.get("questions"); const year = params.get("year");
    if (!ids || !year) return;
  ...
  return (
    <main className="min-h-screen bg-base text-ink-primary">
        ...
        <div className="relative z-10 mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16 max-w-[1200px] text-center">
          <div className="flex justify-center items-end gap-2 mb-6">
            {[
              ...
              <div key={i} className={`w-10 ${book.height} rounded bg-gradient-to-t ${book.color} shadow-lg animate-bounce`} style={{ animationDelay: book.delay, animationDuration: "3s" }} />
            ))}
          </div>
          <h1 className="font-playfair text-[clamp(1.8rem,5vw,3rem)] font-bold leading-tight mb-4">
            Τράπεζα Θεμάτων<br /><span className="text-gold">Πανελλαδικών Εξετάσεων</span>
          </h1>
          <p className="text-ink-tertiary text-lg md:text-xl max-w-2xl mx-auto">Μαθηματικά 1983 – 2025 • ΓΕΛ • ΕΠΑΛ • Επαναληπτικές • Ομογενείς</p>
          <div className="flex items-center justify-center gap-6 md:gap-10 mt-8 text-[0.75rem] tracking-widest text-ink-muted">
            <div className="text-center"><div className="text-2xl md:text-3xl font-bold text-gold font-playfair">43</div><div className="uppercase mt-1">Έτη</div></div>
            <div className="w-px h-8 bg-border-dim" />
            <div className="text-center"><div className="text-2xl md:text-3xl font-bold text-ink-primary font-playfair">1000+</div><div className="uppercase mt-1">Θέματα</div></div>
            <div className="w-px h-8 bg-border-dim" />
            <div className="text-center"><div className="text-2xl md:text-3xl font-bold text-gold font-playfair">4</div><div className="uppercase mt-1">Τύποι Σχολείων</div></div>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto px-4 sm:px-6 lg:px-8 py-4 max-w-[1200px]">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 mb-4">
            <div className="lg:col-span-2">
              <label className="block text-[0.7rem] tracking-wider uppercase text-ink-muted mb-1.5 font-medium">🔍 Αναζήτηση</label>
              <div className="relative">
                <input type="text" placeholder="Αναζήτηση σε κείμενο θέματος..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSearch()} className="w-full bg-card border border-border-dim rounded-lg px-3 py-2.5 pl-10 text-sm text-ink-primary placeholder:text-ink-muted focus:outline-none focus:border-gold/50 transition-colors" />
                <svg className="absolute left-3 top-2.5 w-4 h-4 text-ink-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
              </div>
            </div>
            <div>
              <label className="block text-[0.7rem] tracking-wider uppercase text-ink-muted mb-1.5 font-medium">📅 Έτος</label>
            ...
            <div>
              <label className="block text-[0.7rem] tracking-wider uppercase text-ink-muted mb-1.5 font-medium">🏫 Τύπος Σχολείου</label>
              <select value={selectedSchool} onChange={(e) => setSelectedSchool(e.target.value)} className={selectClass} style={selectStyle}>
                {SCHOOL_OPTIONS.map((s) => <option key={s.value} value={s.value}>{s.label}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-[0.7rem] tracking-wider uppercase text-ink-muted mb-1.5 font-medium">🎯 Θέμα</label>
              <select value={selectedTopic} onChange={(e) => setSelectedTopic(e.target.value)} className={selectClass} style={selectStyle}>
          ...
          <div className="flex gap-3 justify-center">
            ...
            <button onClick={handleReset} className="px-6 py-2.5 bg-white/5 text-ink-muted border border-border-dim rounded-lg font-semibold text-sm hover:border-gold/30 hover:text-ink-secondary transition-all">🔄 Επαναφορά</button>
          </div>
        </div>
      </section>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-6 max-w-[1200px]">
        {filteredProblems.length === 0 ? (
          <div className="text-center py-20">
            ...
            <h3 className="font-playfair text-xl text-ink-primary mb-2">{allProblems.length === 0 ? "Επιλέξτε έτος και πατήστε Αναζήτηση" : "Δεν βρέθηκαν θέματα"}</h3>
              ...
              <div key={problem.id} className="group rounded-xl border border-border-dim bg-card p-5 md:p-6 hover:border-gold/20 transition-all duration-300 relative overflow-hidden">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 mb-4 pl-3">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="px-2.5 py-1 rounded-full text-[0.65rem] font-bold bg-cat-green/10 text-cat-green border border-cat-green/30">{problem.subject}</span>
                  </div>
                ...
                <h3 className="font-playfair text-lg font-semibold text-ink-primary mb-3 pl-3">Θέμα {GREEK_LETTERS[problem.topic] || problem.topic}</h3>
      ...
      <section className="mx-auto px-4 sm:px-6 lg:px-8 py-10 max-w-[1200px] border-t border-border-dim">
        <h2 className="font-playfair text-2xl font-bold text-ink-primary mb-6 flex items-center gap-2">🧮 <span className="text-gold">Το διαγώνισμά μου</span></h2>
                ...
                <div key={q.id} className="border-b border-border-dim pb-6 last:border-0">
                  <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-semibold text-ink-primary">Θέμα {GREEK_LETTERS[q.topic] || q.topic}</span>
                      <span className="text-sm text-ink-muted">({q.year} – {q.school.toUpperCase()})</span>
                    </div>
                    <button onClick={() => removeFromExam(q.id)} className="px-3 py-1.5 bg-red-500/10 text-red-400 border border-red-500/30 rounded-lg text-sm hover:bg-red-500/20 transition-colors">❌ Αφαίρεση</button>
                  </div>
                  <div className="text-sm text-ink-secondary leading-relaxed" dangerouslySetInnerHTML={{ __html: renderContent(q.content) }} />
                </div>
              ))}
            </div>
          )}
      ...
      <footer className="border-t border-border-dim bg-black/50 mt-auto">
        <div className="mx-auto px-4 sm:px-6 lg:px-8 py-8 max-w-[1200px] text-center">
          ...
          <p className="text-[0.8rem] text-ink-muted mt-2">Πανελλαδικές Εξετάσεις 1983 – 2025</p>
