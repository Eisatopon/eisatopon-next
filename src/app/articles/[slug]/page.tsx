...
function renderText(text: string) {
  ...
  return parts.map((part, i) => {
    if (part.startsWith("**") && part.endsWith("**")) {
      return (
        <strong key={i} style={{ color: "var(--color-ink-primary)", fontWeight: 600 }}>
          {part.slice(2, -2)}
        </strong>
      );
    }
    if (part.startsWith("*") && part.endsWith("*")) {
      return (
        <em key={i} style={{ fontStyle: "italic", color: "#c8c4bc" }}>
          {part.slice(1, -1)}
        </em>
      );
    }
        ...
        <article style={{ fontFamily: "var(--font-family-serif)", fontSize: "1.1rem", lineHeight: 1.85, color: "#ccc9c0" }}>
          {paragraphs.map((para, i) => {
            ...
            if (para.startsWith("**") && para.endsWith("**") && para.includes("->")) {
              return (
                <div key={i} style={{ background: "#0a0c14", border: "0.5px solid var(--color-gold-border)", borderRadius: "8px", padding: "16px", margin: "1.8em 0", textAlign: "center", fontFamily: "var(--font-family-mono)", fontSize: "1.1rem", color: "#c4b890", lineHeight: 1.8 }}>
