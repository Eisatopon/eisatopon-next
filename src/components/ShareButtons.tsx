...
export default function ShareButtons({ title, summary }: ShareButtonsProps) {
  ...
  const url = `https://eisatopon.gr${pathname}`;
  ...
  const shareLinks = [
    ...
    },
    {
      ...
      color: "hover:bg-gold/20 hover:text-gold hover:border-gold/50"
    },
  ];

  return (
    <div className="flex items-center gap-2 mt-8 pt-6 border-t border-border-dim">
      <span className="text-[0.75rem] text-ink-muted mr-2">Share:</span>
      {shareLinks.map((link) => (
        <a
          key={link.name}
          href={link.href}
          ...
          className={`flex items-center justify-center w-9 h-9 rounded-lg border border-border-dim bg-white/5 text-ink-secondary transition-all duration-200 ${link.color}`}
        >
          {link.icon}
        </a>
      ))}
